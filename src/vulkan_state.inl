                                    ...
                                    Fn &&submit_func)
{
    ...
    if (!enable_rt_) {
        dev.dt.cmdBindPipeline(render_cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
                       pipeline.rasterState->meshCullPipeline);

        dev.dt.cmdBindDescriptorSets(render_cmd,
                                     VK_PIPELINE_BIND_POINT_COMPUTE,
                                     pipeline.rasterState->meshCullLayout, 0,
                                     1, &frame_state.cullSet,
                                     0, nullptr);

        dev.dt.cmdBindPipeline(render_cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                               pipeline.rasterState->gfxPipeline);

        dev.dt.cmdBindDescriptorSets(render_cmd,
                                     VK_PIPELINE_BIND_POINT_GRAPHICS,
                                     pipeline.rasterState->gfxLayout, 0,
        ...
        for (uint32_t mini_batch_idx = 0; mini_batch_idx < num_mini_batches_;
            ...
            for (uint32_t local_batch_idx = 0;
                 local_batch_idx < mini_batch_size_;
                 local_batch_idx++) {
                ...
                dev.dt.cmdBindDescriptorSets(render_cmd,
                    VK_PIPELINE_BIND_POINT_COMPUTE,
                    pipeline.rasterState->meshCullLayout, 1,
                ...
                dev.dt.cmdPushConstants(render_cmd,
                                        pipeline.rasterState->meshCullLayout,
                ...
                dev.dt.cmdDispatch(render_cmd,
                    getWorkgroupSize(frame_state.maxNumDraws[batch_idx]),
                                     1, 1);
            }
            ...
            for (uint32_t local_batch_idx = 0;
                ...
                if (scene.materialSet.hdl != VK_NULL_HANDLE) {
                    dev.dt.cmdBindDescriptorSets(render_cmd,
                        VK_PIPELINE_BIND_POINT_GRAPHICS,
                        pipeline.rasterState->gfxLayout, 1,
                        1, &scene.materialSet.hdl,
                        0, nullptr);
                }

                glm::u32vec2 batch_offset =
                    frame_state.batchFBOffsets[batch_idx];

                RenderPushConstant render_const {
                    batch_idx,
                };

                dev.dt.cmdPushConstants(render_cmd, pipeline.rasterState->gfxLayout,
        ...
        dev.dt.cmdBindPipeline(render_cmd,
                               VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
                               pipeline.rtState->pipeline);
        ...
        for (uint32_t batch_idx = 0; batch_idx < envs.size(); batch_idx++) {
            ...
            dev.dt.cmdPushConstants(render_cmd, pipeline.rtState->layout,
            ...
            dev.dt.cmdTraceRaysKHR(
                render_cmd,
                &pipeline.rtState->bindingTable.raygenEntry,
                &pipeline.rtState->bindingTable.missEntry,
                &pipeline.rtState->bindingTable.hitEntry,
                &pipeline.rtState->bindingTable.callableEntry,
